...
def establish_max_tvalue_nonfw(w, candidates, ballots, winners_on_fp, \
    min_tvs, aud_tvs, ag_matrix, maxtv, INVALID, args, seats, curr_asn):
    ...
    tr_tv_ub = min(max(asn, curr_asn), args.max_samples)
    while max_tr_tv_ss > tr_tv_ub and bar + delta < tot_vote:
        ...
        if c_in and c_idx < o_idx:
            ...
            if weight == 0:
                ...
                is_ag, ag_asn, descs = rule_out_for_max(\
                    b.prefs, c, ag_matrix, nl_matrix, winners)
...
def compute_iqx(candidates, ow_i, ag_matrix, ballots, INVALID, args, 
    seats, min_tvs, winners_on_fp, winners):
...
def inner_loop(winners_on_fp, args, seats, candidates, cands, valid_ballots,\
    ...
    for ow_i in ows:
        ...
        success, ss_iqx, iqx_assertions, info = compute_iqx(candidates, \
            ow_i, ag_matrix, ballots, INVALID, args, seats, min_tvs, winners_on_fp,\
            winners)
    ...
    for nv in newly_verified:
        max_tr_tv, max_tr_tv_asn, max_tr_tv_aset, max_tr_tv_info = \
            ...
            min_tvs, aud_tvs, ag_matrix, maxtv, INVALID, args, seats, ub)
        ...
        while improvement and could_not_verify != []:
            ...
            for ow in could_not_verify:
                ...
                for c in losers:
                    ...
                    for nv in newly_verified:
                        ...
                        s2, asn2, aset2, info2 = form_NL(candidates, c, ow_i, ags, \
                            ...
                            ag_matrix, nl_matrix, winners, mtv, [nv], [], [],args) # elected
                        s3, asn3, aset3, info3 = form_NL(candidates, c, ow_i, ags, \
                            nls, ballots, INVALID, winners_on_fp, min_tvs, aud_tvs, \
                            ag_matrix, nl_matrix, winners, mtv, [], [],[nv],args) # standing
                ...
                if successNL:
                    ...
                    max_tr_tv, max_tr_tv_asn, max_tr_tv_aset, max_tr_tv_info = \
                            establish_max_tvalue_nonfw(ow, candidates, ballots, winners_on_fp,\
                            min_tvs, aud_tvs, ag_matrix, maxtv, INVALID, args, seats, ub)

                    desc += max_tr_tv_info
                    nv_maxtvs[ow] = (max_tr_tv, max_tr_tv_asn, max_tr_tv_aset)
